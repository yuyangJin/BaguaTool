...
analysis.setStaticAnalysisMode(mode="binary", binary="./memxct")
...
for np in nps:
    ...
    #analysis.setExecutionCommand(cmd="srun -n " + str(np) + " -w gorgon[7,8] ./memxct stmv 100 2.0 1.0 2 300", output_file_suffix="_np-" + str(np))
    analysis.setExecutionCommand(cmd="srun -n " + str(np) + " ./memxct", output_file_suffix="_np-" + str(np))

# set
#analysis.setOutputDir(output_dir="moleculardynamics-baguatool-data")
analysis.setOutputDir(output_dir="memxct-baguatool-data")

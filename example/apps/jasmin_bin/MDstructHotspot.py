...
analysis.setStaticAnalysisMode(mode="binary", binary="./moleculardynamics-main2d")  # , func="conspatch_")
...
nps = [1]
domain_boxes = [1024]#[128, 256, 512, 1024]
patch_sizes = [32]#[4, 8, 16, 32, 64, 128]
for np in nps:
    for domain_box in domain_boxes:
        for patch_size in patch_sizes:
            # set commands for execution
            # # set suffix of output file for performance recording
            analysis.setExecutionCommand(cmd="mpirun -np " + str(np) + " ./moleculardynamics-main2d ./md_input/moleculardynamics-2d.input-p" + str(patch_size) + "-d" + str(domain_box) + "-n" + str(np), output_file_suffix="_np" + str(np) + "-db" + str(domain_box) + "-ps" + str(patch_size), save_file = "MolecularDynamics-2d.log")
...
loop_stack = []
formulation = "T-moleculardynamics = "

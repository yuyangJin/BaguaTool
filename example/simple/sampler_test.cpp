...
//static void (*original_GOMP_parallel)(void *(*fn) (void *), void *data, unsigned num_threads, unsigned int flags) = nullptr;
...
static void DumpAddrLog(bool last_flag){
  if (first_flag == true) {
	...
	for(int i = 0; i < NUM_EVENTS; i++){
      ...
      fprintf(fp[i], "%s | %lld | %d\n", call_path_addr_log[i][j].call_path_str, call_path_addr_log[i][j].count); // , call_path_addr_log[i][j].thread_id);
      fflush(fp[i]);
		}
		call_path_addr_log_pointer[i] = __sync_and_and_fetch(&call_path_addr_log_pointer[i], 0);
	}
...
void RecordCallPath(int y){
  ...
  for (int i = 0; i < call_path_addr_log_pointer[y]; i++){
    if (strcmp(call_path_str, call_path_addr_log[y][i].call_path_str) == 0 ) { // && call_path_addr_log[y][i].thread_id == thread_gid){

...
void ConnectCallerCallee(core::ProgramAbstractionGraph* pag, int vertex_id, void* extra) {
    ...
    std::map<std::string, core::ProgramAbstractionGraph*>* func_name_2_pag = (std::map<std::string, core::ProgramAbstractionGraph*>*)extra;

    if (pag->GetVertexAttributeNum("type", vertex_id) == core::CALL_NODE) {
        core::ProgramAbstractionGraph* callee_pag = (*func_name_2_pag)[std::string(pag->GetVertexAttributeString("name", vertex_id))];
...
core::ProgramAbstractionGraph* Preprocess::InterProceduralAnalysis(std::vector<core::ProgramAbstractionGraph*> &func_pag_vec) {

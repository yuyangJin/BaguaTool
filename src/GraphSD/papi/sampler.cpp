...
void Sampler::GetBacktrace(char* call_path_str,  int max_call_path_str_len) {sa->GetBacktrace(call_path_str,  max_call_path_str_len); }
...
void SamplerImpl::SetOverflow(void (*FUNC_AT_OVERFLOW)(int)) {
  int Events[NUM_EVENTS];
    ...
    //PAPI_overflow_handler_t *_papi_overflow_handler = (PAPI_overflow_handler_t*) &(this->papi_handler) (int EventSet, void *address, long_long overflow_vector, void *context );  
...
void SamplerImpl::Start() {
    TRY(PAPI_start(this->EventSet), PAPI_OK);
}

void SamplerImpl::Stop() {
    TRY(PAPI_stop(this->EventSet, NULL), PAPI_OK);
}
...
void SamplerImpl::GetBacktrace(char* call_path_str,  int max_call_path_str_len) {
				...
				if(addr_log_pointer > 0){
					for (i = 0; i < addr_log_pointer; ++ i){
						offset+=snprintf(call_path_str + offset, max_call_path_str_len - offset - 4 ,"%lx ",(long unsigned int)address_log[i]);
						//LOG_INFO("%08x\n",address_log[i]);
					}
				}
#else

...
void readMPIInfo(string file_name, int pid){
  ...
  for (string line; getline(inputStream, line); ){
    ...
    else if (!type_str.compare(string("s")) || !type_str.compare(string("S")) || !type_str.compare(string("r")) || !type_str.compare(string("R")) || !type_str.compare(string("w"))){
...
bool existCDE(int dest_type, int src_type, string dest_callpath, string src_callpath, int dest_pid, int src_pid){
  for (auto cde : comm_dep_edge){
    if (cde->dest_type == dest_type && cde->src_type == src_type && !cde->dest_callpath.compare(dest_callpath) && !cde->src_callpath.compare(src_callpath) && cde->dest_pid == dest_pid && cde->src_pid == src_pid){
      return true;
    }
  }
  return false;
...
//output inter-process communication dependence edge : (dest , src , edge value) -> (type | call path | pid , type | call path | pid , exe_time)
...
int main(int argc, char* argv[]){
  ...
  //   cout << p2p_info[0][i]->request_count << " ";
  //   cout << p2p_info[0][i]->source[0]  << " ";
  //   cout << p2p_info[0][i]->dest[0] << " ";
  //   cout << p2p_info[0][i]->tag[0] << endl;
    
  // }
  // for (auto& p : p2p_info[0]){
  //   cout << p.first << " ";
  //   cout << p.second->request_count << " " ;
  //   cout << p.second->source[0] << " ";
  //   cout << p.second->dest[0] << " ";
  //   cout << p.second->tag[0] << endl;
  // }

  // for (auto p : trace_log[0]){
  //   cout << p << " ";
  // }
  // cout << endl;

...
map<RequestConverter, pair<int, int>> request_holder;
...
{{fn func MPI_Isend}}{
    ...
    {{callfn}}
		...
		request_holder[RequestConverter(request)]=pair<int, int>(mpiRank, tag);
#ifdef DEBUG
		printf("%s\n", "{{func}}");
#endif
		TRACE_P2P('S', 1, source_list, dest_list, tag_list, time);
...
{{fn func MPI_Waitall}}{
		...
		for (int i = 0 ; i < count; i ++){
			...
			pair<int, int> p1 = request_holder[RequestConverter(&array_of_requests[i])];
			//if (mpiRank == 0){
			printf("find %d %d\n", p1.first, p1.second);
			cout << "waiting("<<mpiRank<<")-> "<<p1.first << " " << p1.second<<std::endl;
			//}

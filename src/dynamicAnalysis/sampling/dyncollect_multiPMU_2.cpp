...
#define ADDRLOGSIZE 60000
#define MAX_STACK_DEPTH 100
#define MAX_CALL_PATH_LEN 800 // MAX_STACK_DEPTH * 8
#define NUM_EVENTS 2
...
static void write_addr_log(){
	...
	for(int i = 0; i < NUM_EVENTS; i++){
		ofstream outputStream((string("SAMPLE") + to_string(mpiRank) + string("-") + to_string(i + 4) + string(".TXT")), ios_base::app);
...
void static set_papi_overflow()
{
  ...
  //Events[1] = PAPI_L2_DCM;
	...
	Events[1] = PAPI_L3_DCA;


	TRY(PAPI_add_events(EventSet, (int *)Events, NUM_EVENTS), PAPI_OK);
	TRY(PAPI_overflow(EventSet, PAPI_TOT_INS, INS_SAMPLE_COUNT, 0, papi_handler), PAPI_OK);
	...
	//TRY(PAPI_overflow(EventSet, PAPI_L2_DCM, CM_SAMPLE_COUNT, 0, papi_handler), PAPI_OK);
